...
For a more salient example, consider the actuary's task of estimating the prospective loss ratio for a given book of business when they only have two years of data. If the industry loss ratios and the company's loss ratios for that exposure are both consistently in the 75-80% range, one might expect the prospective loss ratio for your book and the industry to be similar. However, the volatility in loss ratios will be greater for your book than for the industry.
...
This is the concept that if we observe a higher-than-average value sampled from a known distribution, that one should expect the next sample to be closer to average. However, studies show that many subjects do not always recognize this phenomenon. They can either fail to believe regression is applicable in their specific scenario or they might "invent spurious causal explanations."
...
Two common schools of statistical thinking are 1) Frequentist and 2) Bayesian.
...
| End Result           | Probability of observing events based on a distribution given a set of parameters | Describe parameters probabilistically based on observations and prior assumptions |
...
-   A **Markov process** is a random process for which the future outcomes depend only on the present state.

-   **Monte Carlo** methods use random sampling to approximate an outcome.

-   Putting **Markov process** and **Monte Carlo** methods together, MCMC is a way to gather a representation of the posterior distribution via simulation and repeated sampling.

It is easy to think about these two items separately, but what might an example of this look like in action?

::: {custom-style="Highlight"}
This section needs to cite the source of the example. There are also a few outstanding comments from Raj.
:::

Imagine there is a single frog on a pond covered in lily pads. The frog can jump from one lily pad to any of the other lily pads in the pond. The frog is more likely to jump to lily pads that are closer to it, but it can certainly hop all the way across the pond to another lily pad. The frog wants to find a good lily pad to sit on and get sunlight.
...
rstan::stan(
  ...
  model_code = '
      ...
      // Simulated parameters - i.e., where you set up prior distribution
      parameters {
        // Poisson parameter of our likelihood distribution
        real<lower = 0> sampled_lambda;
      }
      
      // Main model
      model {
        // Assumed prior distribution of lambda: ChiSquare(nu = 2)
        sampled_lambda ~ chi_square(2); 
        
        // For each data point, we simulate a poisson likelihood
        for(i in 1:N) {
          obs[i] ~ poisson(sampled_lambda);
        }
      }
  '
)
...
-   Effective Sample Size (N-eff) tests the correlation of the sampling done in the simulation. Since it is a Markov chain and the next iteration is dependent on the current starting position, there is going to be correlation between samples. The effective sample size statistic helps identify how much correlation occurs during the sampling through the following formula:
    ...
    ::: {custom-style="Highlight"}
    Outstanding comments from Grant on n_eff
    :::
...
-   Trace plots show the results of the MCMC model in sequential order of the simulation. Since the output of our model is parameter estimates, we would create a trace plot for each parameter with the iteration number as the x-axis and the corresponding simulated parameter value as the y-value and draw a connecting line so we can see the path of the simulation. Ideally, we would want a trace plot to appear random, like white noise, as a sign of well-mixing models and well-converged chains. Below comparing the left and right plot, the left plot is an example of well-mixed trace plot result, the points appear random, with no specific pattern in the lines, and all 3 chains are converging over the same region. The plot on the right is a not well-mixed model, as we see the orange line has a much higher mean across all iterations than the pink or blue lines.
    ...
    ::: {custom-style="Highlight"}
    Outstanding comments from Grant on converging
    :::
...
-   Regression to the mean: As seen in the chart below, the high average for 2007-2011 was followed by a low average in 2012-2023. This resulted in the new all-year average (1983-2023) being nearly identical to the all-year average before the rise (1983-2006).
