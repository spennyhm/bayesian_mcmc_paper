...
author: <p>Spencer Miller<span style='font-variant:small-caps'>, fcas, maaa</span> & Kenneth Smart<span style='font-variant:small-caps'>, fcas, maaa</span></p>
date: '`r Sys.Date()`'
...
Much has been written in the actuarial literature about Bayesian statistical methods over the last couple decades, few of which are still part of current exam syllabi. While these papers provide a thorough explanation of the process, the math can seem overly complex even to an experienced actuary. The purpose of this paper is twofold. First, we want to provide property & casualty actuaries with a *basic* understanding of what goes on behind the scenes of a Bayesian model without getting bogged down in the underlying calculations. Second, we want to champion the continued inclusion of these topics on exam syllabi in the future.

While we believe this paper is beneficial to everyone in the profession, we view the following as the target audience:

-   An actuary who finished exams a few years ago and wants to ease their way into Bayesian methods.
-   An actuary who occasionally finds their estimates to be out of line with expectations, with no discernible reason.
-   An actuary who is interested in learning about Bayesian methods but finds the math too onerous or intimidating.
...
In Section 4, we give a primer on the basics of Bayesian statistics and introduce Markov Chain Monte Carlo (MCMC) methods.
...
This paper was written using R Markdown. The underlying code is available as a supplementary file. Where beneficial, we show the code in the body of the paper. Sample code for setting up a possible model is included at the end of the paper.

# 2. A Lesson in Behavioral Economics

When making predictions, people often rely on mental operations ("heuristics"). While these may be useful from an evolutionary standpoint, they can lead to "... systematic and predictable errors". Below is a non-exhaustive list of possible factors at play when making actuarial predictions, along with key examples from "Judgment Under Uncertainty: Heuristics and Biases" by Tversky & Kahnemann.
...
The majority of subjects said the numbers of days would be about the same, while the minority of subjects were split evenly between the larger and smaller hospitals. According to sampling theory, there will be more such days in the smaller hospital since smaller populations generally experience more volatility.

For a more salient example, consider the actuary's task of estimating the prospective loss ratio for a given book of business when they only have two years of data. If the industry loss ratios and the company's historical loss ratios for that exposure are both consistently in the 75-80% range, one might expect the prospective loss ratio for your book and the industry to be similar. However, the volatility in loss ratios will be greater for your book than for the industry.
...
For example, when subjects were tasked with evaluating the likelihood of whether a randomly sampled person out of a pool of 30 engineers and 70 lawyers was a lawyer or an engineer, subjects correctly identified the relative likelihoods (30% & 70%). However, when the following description of the randomly sampled person was introduced, subjects "evaluated the likelihood that a particular description belonged to an engineer rather than to a lawyer ... with little or no regard for the prior probabilities of the two outcomes."

> Dick is a 30-year old man. He is married with no children. A man of high ability and high motivation, he promises to be quite successful in his field. He is well liked by his colleagues.

When presented with the seemingly uninformative description, subjects now estimated the relative likelihoods to be equal (50% & 50%). That is, they ignored the distribution of professions from the population (30% & 70%). Even though the description offers no clue as to what profession Dick might belong to, subjects ignored the distribution of professions for the populations.

::: {custom-style="Highlight"}
Still need an example. If none come to mind, just might delete this one.
:::

## 2.3 Misconception of Chance

People will expect even a short sample of random events to "look random." For example, when shown two sets of a random sequences of fair coin tosses, subjects will judge "HTHTTH" to be more likely than "HHHTTT", despite the two sets being equally likely.

For an example from our profession, consider the task of selecting trend rates for a company's given book of business. While we try to incorporate external information in our selections (hard versus soft market, inflation, changes in business practices, etc.), there will always be a tendency to "eyeball" trend lines - even when the data points have no clear trend. In other words, is that trendline actually increasing or are those last few points elevated merely by chance?

While we would like to think that actuaries would be immune to this particular bias, even experienced research psychologists believe they can reach a valid hypothesis by relying on small samples deemed "highly representative of the populations from which they are drawn." As a result, they "... put too much faith in the results of small samples, and grossly overestimated the replicability of such results."

## 2.4 Insensitivity to Predictive Accuracy

When making predictions, studies show that when given only a favorable or unfavorable description, subjects' predictions were "insensitive to the reliability of the description." In other words, people tend to take a description at face value, with little regard to the credibility of the description.

Consider the following example. You are tasked with estimating the average ultimate severity of claims occurring during the upcoming policy year. The recently hired risk manager tells you that there are new post-loss measures being put in place that will reduce the potential of large claims. Taking the risk manager’s assessment of the new claims handling process without support could lead to deficient estimates.

## 2.5 Regression to the Mean

If we observe a higher-than-average value sampled from a known distribution, then one might expect the next sample to be closer to average. However, studies show that many subjects do not always recognize this phenomenon. They can either fail to believe regression is applicable in their specific scenario or they might "invent spurious causal explanations."

In the case of forecasting losses, one might experience this phenomenom. Suppose losses are generally stable year-over-year, but the most recent year unexplicably dropped 20% relative to the prior year. How much weight should we give this possible outlier? Do we give the most weight to the average? While there is no right answer, being aware of this potential bias will allow you to make a more well-informed decision.

## 2.6 Anchoring

In one study, subjects were tasked with estimating the percentage of African countries in the United Nations. Before making their estimate, the subjects were asked to spin a wheel with numbers from 0 to 100. The subjects had to adjust upwards or downwards from this starting value. The median estimate for those with a starting value of 10 was 25%. When the starting value was 65, the median estimate increased to 45%. This example illustrates the concept of anchoring, which is where some initial value influences the estimated value.

Suppose you're in charge of providing a loss forecast for the upcoming accident year and have been doing so for a few years. When making your selection, you are acutely aware of the estimate you provided last year (i.e., the "initial value"). This prior estimate might be "anchoring" your estimate of the prospective year. While that may not be a bad thing, issues may arise when there is insufficient adjustment to the anchoring value.

# 3. Statistical Models

## 3.1 Background

The "actuarial central estimate" is a cornerstone of our profession. It reflects the culmination of years of studying and practice. For a lot of our work, it's all that is typically required. After all, we're seen as the experts and you'd be hard-pressed to find an end user of our work that is as concerned about confidence levels as we are.

As we each have likely missed the mark on a projection at some point in our careers, it is worth reflecting on those misses. Were they due to chance (e.g., process risk), did we fail to adequately capture possible alternatives (e.g., model risk), or was there some underlying bias that caused us to see a trend that wasn't actually there? There are a lot of risks that are not captured when solely relying on point estimates. We list a few of these risks below (Venter & Sahasrabuddhe):

-   Process Risk: Inherent uncertainty of the insurance claims process.
-   Sampling Risk: Differences between observed sample and the population.
-   Data Bias: Results from incorrectly adjusting data in experience period to be on same basis (e.g., cost level) as the exposure period.
-   Model Misspecification: The risk that one did not select the right model.
-   Insufficient Parameter Identification: Results when we fail to consider the uncertainty of our model elements.

## 3.2 Statistical Schools of Thought

Two common schools of statistical thinking are Frequentist and Bayesian.
...
Bayesian statistics may sound familiar to you, as the core of Bayesian statistics is an idea called Bayes' theorem, which most actuaries would have learned in an introductory statistics course.
...
|-----------------|-----------------------------|--------------------------|
| Observed Data        | Repeatable and random events                                                                            | Known and fixed sample                                                                       |
| Parameters           | Fixed values                                                                                            | Unknown                                                                                      |
| View on Probability  | Objective                                                                                               | Subjective                                                                                   |
| Computational Burden | Minimal                                                                                                 | Significant                                                                                  |
| End Result           | The probability of observing a set of events is based on a distribution and a set of defined parameters | Describes the parameter spaces probabilistically based on observations and prior assumptions |

Upon reviewing these fundamentals, we would argue that actuarial math (as with most predictive statistics) is inherently Bayesian at its core. As actuaries, we typically know historical claim information and can say that claims and events are a known and fixed sample of observations. It is important to note that a lack of historical claims (zero observations) can also be relevant information when modeling.

When predicting future claims or development on claims, we don’t know for certain the parameters of the future. However, given the historical information and our expertise we can describe some basis of prior assumption about the parameters. Finally, as we want to capture as much risk in our modelling as possible, a frequentist approach typically lacks parameter risk if parameters are fixed.

As such, Bayesian statistics can greatly improve actuarial models by incorporating parameter risk without really changing our view on known information. Bayesian methods allow for the incorporation of prior knowledge (or what we like to call actuarial judgement) into our prior assumptions to influence our models. Lastly, technological advances have made Bayesian modeling more accessible by minimizing the computationally intensive hurdles.

# 4 Bayesian Statistical Modelling

## 4.1 Basics of Bayesian Statistics

As mentioned previously, the foundation of Bayesian statistics is Bayes' theorem. To give a brief refresher on what Bayes' theorem is: Bayes' theorem describes how to calculate an updated assumption (prior) beginning with a prior assumption and accounting for new information provided (historical evidence). It is represented by the formula below. Starting with the traditional definition where X and Y are some events and transforming it into our context where we have some observed data and $\theta$, which represents the parameter space we intend to estimate.

$P(Y|X) = \frac{P(X|Y)}{P(X)} = \frac{f(X|Y)(f(Y)}{\int_{-\infty}^{\infty} f(X|Y)(f(Y)dy} = \frac{f(data|\theta)*f(\theta)}{\int f(data|\theta)*f(\theta)d\theta} = P(\theta|data)$
...
4.  The posterior $P(\theta|data)$ — the result, which is the modified probability of the parameter space after accounting for the data.
...
::: {custom-style="Highlight"}
Can we revise this chart to have a wide and narrow prior? We need to have a small blurb on the difference.
:::
...
MCMC methods are an effective way of using Bayesian statistics in actuarial models. By effective, we mean that these methods can replace the need to deal with the complex integral in the denominator of Bayes' theorem. Lets start by breaking down the term Markov Chain Monte Carlo:

-   A **Markov chain** is a random process for which the future outcomes depend only on the present state.

-   **Monte Carlo** methods use random sampling to approximate an outcome.

-   Putting **Markov chain** and **Monte Carlo** methods together, MCMC is a way to gather a representation of the posterior distribution via simulation and repeated sampling.

It is easy to think about these two items separately, but what might an example of this look like in action?

::: {custom-style="Highlight"}
There are also a few outstanding comments from Raj.
:::

Imagine there is a single frog on a pond covered in lily pads. The frog can jump from one lily pad to any of the other lily pads in the pond. The frog is more likely to jump to lily pads that are closer to it, but it can certainly hop all the way across the pond to another lily pad.

Which lily pad the frog jumps to from its current lily pad is random, but there is a higher probability that the frog jumps to a lily pad that is closer to it than one that is farther away. Essentially, the next lily pad that the frog jumps to has a distribution around it that is dependent only on the lily pad that the frog is currently on (i.e., this is a Markov chain).

Now, when the frog jumps to a new lily pad, the decision to stay on that lily pad is dependent on how much the frog likes that lily pad. The frog likes a lily pads based on how much sunlight it gets there (i.e., this is a likelihood function). If the frog doesn’t like how much sunlight it gets at the new lily pad, imagine that the frog forgets that it ever jumped to it and goes back to their previous lily pad before jumping again.

It’s an indecisive frog so it doesn’t want to stay on the same lily pad, and so the frog will keep jumping from lily pad to lily pad (i.e., this a Monte Carlo process). At the end of the day, the frog will have spent more time on lily pads that got the most sunlight, and less time at those in the shade that weren’t the right fit for the frog.

Hopefully, this is starting to paint a picture of what an MCMC process looks like and how the Markov chain and Monte Carlo simulation pieces fit together.

Extrapolating this example out to statistical terms, if we think about the lily pads as being different parameter sets, the frog jumps around from parameter set to parameter set trying to find one that fits the frog’s needs well. We can think about the frog liking a certain lily pad as how well the parameter set (e.g., the given lily pad) describes the observed data (e.g., the amount of sunlight). This is just a likelihood function (i.e., what is the likelihood of the data given the parameter set). In an MCMC simulation, if that likelihood is above or below some random simulated threshold, we will accept or reject the parameter set (e.g., will the frog stay at that lily pad or go back to the one it jumped there from). Finally, if the frog continues to jump around repeatedly, we observe which lily pads the frog spends the most time on, we can end up with a distribution of which lily pads the frog liked best. In statistical terms, this equates to which parameter sets that are most likely to describe the data.

This example is a version of the Metropolis-Hastings algorithm, which is a form of an MCMC process. There are several forms of MCMC processes: Gibbs sampling, Metropolis-Hastings, Hamiltonian Monte Carlo, etc. The specifics of how each one works are beyond the scope of this paper.

## 4.3 Execution of MCMC

MCMC can be executed using Stan, an open-source software, via common coding languages (e.g., Python and R). Later in this paper we dive into a comparison of MCMC fits verses traditional methods and provide a real example of fitting an MCMC process. In this paper, we leverage the RStan package, though there are numerous other packages that can be used (RStan leverages Hamiltonian Monte Carlo).
...
::: {custom-style="Highlight"}
I like Alex's idea of having tying this back to anchoring
:::

::: {custom-style="Highlight"}
See Jaylen's comment on the above paragraph
:::
...
For our example, let's assume you are tasked with estimating the expected frequency of global earthquakes over 6.0 magnitude for the year 2012. You are given a dataset of prior earthquakes since 1983.
...
gg_orig <- ggplot2::ggplot() +
  ggplot2::scale_x_continuous(name = 'Year', breaks = seq(1983, 2023, 3)) +
  ...
  ggplot2::geom_hline(
    mapping = ggplot2::aes(
      yintercept = point_estimate,
      color = 'Five-Year Average'
    )
  ) +
  ggplot2::geom_hline(
    mapping = ggplot2::aes(
      yintercept = mean(counts$count),
      color = 'All-Year Average'
    )
  ) +
  ggplot2::geom_text(
    mapping = ggplot2::aes(
      x = 1987,
      y = point_estimate,
      label = paste0('Point Estimate = ', round(point_estimate, 2))
    ),
    vjust = -0.5,
    size = 3
  )
...
In the simplest case, you may decide given the recent trend, that the five-year average is the best approximation of future frequency. In this case, your selected frequency is `r round(point_estimate, 2)` ("Point Estimate"). This approach is consistent with the frequentist methods described earlier and can be written as follows:

$X = `r round(point_estimate, 2)`$

Your next logical step might be to add some process risk around your estimate by fitting a distribution to the data. Based on an examination of the data, you decide to model frequency using the Poisson distribution with lambda = `r round(point_estimate, 2)`. This can written as follows:

$X \sim Pois(\lambda = `r round(point_estimate, 2)`)$
...
Another possible step would be to layer in parameter risk since we might not be convinced of our selected lambda. You decide on letting lambda follow a chi-square distribution with degrees of freedom equal to your point estimate. This can written as follows:

$X \sim Pois(\lambda \sim ChiSquare(d.f. = `r round(point_estimate, 2)`))$
...
After running 10,000 simulations with your selected distribution, you're left with a central estimate of `r round(bayesian, 2)`. Since you took an objective view (i.e., "wide", "uninformative", "weak", "vague") of your prior distribution, your result will be closer to the historical average (`r round(mean(counts$count), 2)`) than the point estimate (`r round(point_estimate, 2)`).

If instead you had strong reason to believe that there would be `r round(point_estimate, 2)` earthquakes, you may opt for a "stronger" prior by letting $\lambda$ have a much tighter distribution around the mean.
...
It appears that all four methods severely overestimated the true average frequency. However, if we look at the Bayesian versus the other three, the true count is a more likely outcome. The "true count" is approximately equal to the `r scales::percent(length(bayesian_sample[bayesian_sample <= true_count]) / iterations, accuracy = 0.1)` confidence level of our Bayesian estimate. While this may still seem low, it is relatively close to the proportion of historical counts below `r true_count` (`r scales::percent(length(counts$count[counts$count <= true_count]) / nrow(counts), accuracy = 0.1)`).
...
-   Misconception of chance: Humans just aren't that great at identifying random processes. For example, a series of coin tosses with all heads is seen as "less random" than a series with alternating heads and tails, even though both results are equally likely.
-   Regression to the mean: As seen below, the high average for 2007-2011 was followed by a low average in 2012-2023. This resulted in the new all-year average (1983-2023) being nearly identical to the all-year average before the rise (1983-2006).
...
::: {custom-style="Highlight"}
Need some sort of conclusion. Consider Grant's comments.
:::
...
We believe there are several other possible applications, ranging from the selection of development factors to estimating reserve variability. We leave it to the reader to explore these.
...
Spencer is a Senior Actuary with Spring Consulting Group. He holds a Bachelor of Science from Lebanon Valley College. Spencer is a Fellow of the Casualty Actuarial Society and a Member of the American Academy of Actuaries.

Kenneth is a Senior Manager with Oliver Wyman Actuarial Consulting, Inc. He holds a Bachelor of Science degree in Actuarial Mathematics and Statistics from the University of Pittsburgh. Kenny is a Fellow of the Casualty Actuarial Society and a Member of the American Academy of Actuaries.

## 6.4 Citations

::: {custom-style="Highlight"}
Need Kenny's citations still
:::
...
Venter, Gary, and Rajesh Sahasrabuddhe. 2015. “A Note on Parameter Risk.” Variance 9 (1).
